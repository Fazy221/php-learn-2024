...
function averageSalary($listings) {
  ...
  $averageSal = $sumSalary > 0 ? formatSalary($sumSalary/count($listings)) : 0;
  return $averageSal;
}
// method 2
function averageSalary2($listings) {
  $salariesArr = array_column($listings, 'salary'); // This will take values of all salary keys and put in seperate arr like ['42000', '51000']
  $totalSal = array_sum($salariesArr);
  $averageSal = $totalSal > 0 ? formatSalary($totalSal/count($listings)) : 0;
  return $averageSal;
}
?>
