...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['submit'])) {
  ...
  $sql = 'INSERT INTO posts(title, body) VALUES(:title, :body)'; // Will use :title and :body to avoid getting hacked
  $stmt = $pdo->prepare($sql); // Will put it under prepare to let it cook
  $params = ['title'=>$title, 'body'=>$body]; // put real values in there like we did previously
  $stmt->execute($params); // finally execute with given params
  header('Location: index.php'); // head to certain location after submission
  exit;
}
?>
